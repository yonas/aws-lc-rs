...
if [[ "${MSVC_ARCH}" = x* ]]; then
  dumpbin //EXPORTS //SYMBOLS  "${LIBCRYPTO_PATH}" | grep External | grep -v UNDEF | sed -e 's/.*External\s*|\s*\(.*\)$/\1/' | grep -v -E '^_*v?f?s?n?printf' | grep -v -E '^_*v?s?f?scanf' | grep -E '^\w*$' | sort | uniq >"${SYMBOLS_FILE}"
else
  dumpbin //EXPORTS //SYMBOLS  "${LIBCRYPTO_PATH}" | grep External | grep -v UNDEF | sed -e 's/.*External\s*|\s*\(.*\)$/\1/' | grep -v -E '^_*v?f?s?n?printf' | grep -v -E '^_*v?s?f?scanf' |  sort | uniq | filter_symbols>"${SYMBOLS_FILE}"
fi
